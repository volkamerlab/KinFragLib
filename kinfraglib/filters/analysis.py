...
def count_accepted_rejected(fragment_library, bool_column_name, filtername):
    ...
    accepted = pd.Series(df.get_group(1).groupby('subpocket', sort=False).size(), name ="accepted_"+filtername)
    rejected = pd.Series(df.get_group(0).groupby('subpocket', sort=False).size(), name = "rejected_"+filtername)
...
def count_fragments(fragment_library, name = "n_frags"):
    ...
    pandas.Series
        number of accepted and number of rected fragments per subpocket
    """
    return pd.Series(pd.concat(fragment_library).reset_index(drop=True).groupby('subpocket', sort=False).size(), name = name) 
